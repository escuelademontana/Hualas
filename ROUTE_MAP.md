...
- `/accounting` -> `src/app/accounting/page.tsx`
  - Shows the accounting dashboard. If professor payment or month-close
    migrations are pending in the connected database, the page still renders
    with a warning and omits the unavailable totals.
...
- `POST` `/api/activities/[id]/withdraw` -> `src/app/api/activities/[id]/withdraw/route.ts`
  - Requires an owned active registration, a withdrawal note of at least 50
    characters, and a rating from 0 to 5; payment status does not block the
    withdrawal.
